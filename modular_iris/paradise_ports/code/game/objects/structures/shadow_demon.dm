...
/mob/living/basic/demon/shadow
	...
	//base_lighting_alpha = LIGHTING_PLANE_ALPHA_MOSTLY_VISIBLE // so they can tell where the darkness is
...
/mob/living/basic/demon/slaughter/grant_loot()
	var/static/list/droppable_loot = list(
		///obj/item/organ/heart/demon/shadow,
	)
...
/mob/living/basic/demon/shadow/Life(seconds, times_fired)
	...
	if(lum_count > 0.2)
		adjustBruteLoss(30) // 10 seconds in light
	//	SEND_SOUND(src, sound('sound/weapons/sear.ogg'))
		to_chat(src, span_boldwarning("The light scalds you!"))
	else
		adjustBruteLoss(-20)
...
// /obj/structure/shadowcocoon/Destr()
// 	visible_message(span_notice("[src] splits open, and the shadows dancing around it fade."))
// 	return ..()
...
/mob/living/basic/demon/shadow/Initialize(mapload)
	...
	var/static/list/innate_actions = list(
		...
		//datum/action/cooldown/spell/list_target/telepathy/shadow_demon,
	)
